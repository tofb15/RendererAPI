...
namespace CodeParse {
	...
	/*String*/
	const RegularExp::CharacterSelect reg_name_special_tokens("_");
	const RegularExp::OR   reg_name_part1({&RegularExp::g_letter, &reg_name_special_tokens});
	const RegularExp::OR   reg_name_part2({&reg_name_part1, &RegularExp::g_numberPositive});
	...
	/*Namespace*/
	const RegularExp::CharacterSelect reg_namespace_special_tokens(":");
	...
	/*Pointers*/
	const RegularExp::CharacterSelect reg_pointer("*&");
	const RegularExp::Star reg_pointer_star(reg_pointer);
	
	/*Code Block / Scope*/
	const RegularExp::CharacterSelect reg_scope_start("{");
	const RegularExp::CharacterSelect reg_scope_end("}");
	const RegularExp::AnyExept        reg_anyExept_scope_end("}");
	...
	const RegularExp::AND             reg_fileExtension({ &reg_dot, &reg_string});
	const RegularExp::Star_Min_Max    reg_specialFolderName(reg_dot, 2, 2);
	...
	const RegularExp::CharacterSelect reg_inc1_start("<");
	const RegularExp::CharacterSelect reg_inc1_end(">");
	const RegularExp::CharacterSelect reg_inc2_startEnd("\"");
